  ...
  return (
    ...
    >
      ...
      {hintVisible && !loading && !loadError && (
        <div className="absolute left-3 right-3 top-4 z-10 flex justify-center pointer-events-none">
      ...
      {/* Controles flotantes */}
      {!loadError && (
        <div className="absolute inset-x-3 bottom-4 z-10 flex items-center justify-center gap-2 sm:inset-x-auto sm:right-4 sm:flex-col">
          <button
            type="button"
            onClick={toggleFullscreen}
            aria-label={isFullscreen ? 'Salir de pantalla completa' : 'Pantalla completa'}
            className="inline-flex min-h-11 items-center gap-2 rounded-full border border-white/15 bg-black/55 px-4 text-xs font-bold uppercase tracking-[0.08em] text-white backdrop-blur transition-colors hover:border-[var(--accent,#FA5D29)] hover:bg-[var(--accent,#FA5D29)] sm:px-3"
          >
            {isFullscreen ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
            <span className="sm:hidden">{isFullscreen ? 'Salir' : 'Pantalla'}</span>
          </button>
          <button
            type="button"
            ...
            className={
              'inline-flex min-h-11 items-center gap-2 rounded-full border px-4 text-xs font-bold uppercase tracking-[0.08em] backdrop-blur transition-colors sm:px-3 ' +
              (gyroState === 'on'
                ...
                : gyroState === 'unsupported'
                  ...
                  : 'bg-black/55 border-white/15 text-white hover:bg-[var(--accent,#FA5D29)] hover:border-[var(--accent,#FA5D29)]')
            }
          >
            <Compass className="w-4 h-4" />
            <span className="sm:hidden">{gyroState === 'on' ? 'Sensores on' : 'Mover celu'}</span>
          </button>
        </div>
      )}

      {gyroState === 'denied' && (
        <div className="absolute bottom-20 left-4 right-4 z-10 mx-auto max-w-[260px] rounded-lg border border-white/10 bg-black/70 px-3 py-2 text-center text-[11px] text-white/80 backdrop-blur sm:left-auto sm:right-4 sm:mx-0">
